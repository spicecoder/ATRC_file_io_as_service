...
async function main(containername,containerdir,filename) {
  try {
  ...
  //  let container = client.Container(containers[0].name)
    ...
    for (object of objects){
        console.log("container object name:" + object.name)
        if (object.name.includes(containerdir)) {
          console.log("DIR found" + object.name )
          let dobject = selectedcontainer.Object(object.name)
          //let fos = dobject.list()
          console.log("files in dir:"+ object.name)
          // download file
...
main(containername,containerdir,filename)
