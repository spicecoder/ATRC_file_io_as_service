...
async function main(containername,containerdir,srcdir) {
  try {
  ...
  //  let objects = await selectedcontainer.list()
    ...
    //     //   // download file
    //     //   let dst = fs.createWriteStream("in_" +filename)
    //     //   wobject.write(dst)
 ...
 //loop for files in the srcdir  
 const files = fs.readdirSync(srcdir)
 console.log("src:" + srcdir+ "no of files:"+ files.length)
 for (const filename of files) {
          console.log("looping:"+ filename);
          const src = fs.createReadStream(srcdir + filename)
          console.log("container dir:"+ containerdir);
let oobject = await selectedcontainer.create(containerdir + filename, src)
console.log(oobject); // `SwiftObject` instance
 }}
  } catch (e) {
    console.log("Error", e)
  }
...
main(containername,containerdir,srcdir)
